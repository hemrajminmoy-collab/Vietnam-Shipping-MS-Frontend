              ...
              <div style={{ marginTop: 24, paddingTop: 20, borderTop: "2px solid #f3f4f6" }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, marginTop: 16 }}>
                  ...
                  <button 
                    onClick={async () => {
                      try {
                        // Save all queued containers
                        const promises = queuedContainers.map(q => 
                          axios.post("https://vietnam-shipping-ms-backend-six.vercel.app/api/warehouse", q)
                        );
                        await Promise.all(promises);
                        setRecords((prev) => [...queuedContainers, ...prev]);
